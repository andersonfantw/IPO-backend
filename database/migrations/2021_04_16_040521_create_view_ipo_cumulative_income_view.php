...
class CreateViewIpoCumulativeIncomeView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement("DROP VIEW IF EXISTS view_ipo_cumulative_income");
        $sql=<<<SQL
CREATE VIEW `view_ipo_cumulative_income` AS
select client_acc_id, sum(amt) as cumulative_income from(
select client_acc_id,sum(amount) as amt from a_client_fund_in_out_listing_csv01 where gl_mapping_item_id!='OTH:Acct1' group by client_acc_id
union
select client_acc_id,sum(net_amt) as amt from a_client_trade_journal_w_chrg_details_csv05 group by client_acc_id
) as t
