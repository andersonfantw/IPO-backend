...
class CreateViewAIpoSubscriptionView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement("DROP VIEW IF EXISTS view_a_ipo_subscription");
        $sql=<<<SQL
CREATE VIEW `view_a_ipo_subscription` AS
    SELECT 
        ...
        CONCAT(SUBSTRING_INDEX(SUBSTRING_INDEX(cyss_open_account.a_client_fund_in_out_listing_csv01.remark,':HK',1),' ',-1),':HK') AS product_id,
        SUBSTRING_INDEX(SUBSTRING_INDEX(cyss_open_account.a_client_fund_in_out_listing_csv01.remark,'Qty: ',-1),' ',1) AS qty,
        0 AS alloted,
    ...
    WHERE
        (a_client_fund_in_out_listing_csv01.gl_mapping_item_id = 'OTH:IPOSubscription');
SQL;
        DB::statement($sql);
    }
