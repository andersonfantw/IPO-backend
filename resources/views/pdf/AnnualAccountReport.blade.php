...
<body>
    ...
    <div class="container">
        ...
        <div>
            <b>說明:</b>
            <ol>
                @if ($data['TempIpoSummary']['restore_avail_bal'] - $data['TempIpoSummary']['correct_init_val'] > 0)
                    @if ($data['TempIpoSummary']['current_program']=='C' || strlen($data['TempIpoSummary']['client_acc_id'])===8)
                <li>表現費: {{number_format(($data['TempIpoSummary']['restore_avail_bal'] - $data['TempIpoSummary']['correct_init_val'])*0.2,2)}} ({{number_format($data['TempIpoSummary']['restore_avail_bal'] - $data['TempIpoSummary']['correct_init_val'],2)}}*20%)將於 {{$data['AccountReportSendingSummary']['performance_fee_date']->format('Y/m/d')}} 扣除</li>
                    @else
                <li>表現費: {{number_format(($data['TempIpoSummary']['restore_avail_bal'] - $data['TempIpoSummary']['correct_init_val'])*0.8,2)}} ({{number_format($data['TempIpoSummary']['restore_avail_bal'] - $data['TempIpoSummary']['correct_init_val'],2)}}*80%)將於 {{$data['AccountReportSendingSummary']['performance_fee_date']->format('Y/m/d')}} 扣除</li>
                    @endif
                @endif
