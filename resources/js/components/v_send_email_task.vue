...
export default {
    ...
    methods:{
        get_template_list(){
            ...
            },{mode:(this.mode)?'group_email':'email'},this.url('/template_list'))
        },
        choose_receiver(){
            switch(this.mode){
                case 'group':
        ...
        show_template(){
            ...
            if(item.length==1){
                ...
                this.preview_url = this.api_prefix+'/preview_email/'+this.form.template+((this.form.client_id)?'?client_id='+this.form.client_id:'')
            }
        },
        written_mode(){
