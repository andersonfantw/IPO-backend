<template>
    <b-input-group :size="_component_size">
        <b-input :id="_id" :ref="_id" :size="_component_size" :list="_id+'client_list'" v-model="find_a_client.client_id" @keyup="find_client" :value="value" @input="$emit('input', find_a_client.client_id)" placeholder="四位數字後自動檢索" :disabled="disabled"></b-input>
        ...
        <datalist :id="_id+'client_list'">
            <option v-for="client in find_client_list" :key="client['client_id']" :value="client['client_id']">{{ client['name'] }}</option>
        </datalist>
...
export default {
    ...
    created(){
        this.find_a_client.client_id = this.value
        this.find_client()
    ...
    watch:{
        'find_a_client.client_id': function(n){
            let arr = this.find_client_list.filter(function(i){
                return i.client_id==n
            })
            if(arr.length) this.find_a_client.client_name = arr[0].name
            else this.find_a_client.client_name = ''
            // 沒有button，選定後用事件回傳
            if(!this._showbutton && this.find_a_client.client_name!='') this.$bus.$emit('find_a_client::client',this.find_a_client)
        }
    },
    computed:{
        // prop default
    ...
    methods:{
        ...
        find_client(){
            let _this = this
            if(this.find_a_client.client_id){
                if(this.find_a_client.client_id.length>3){
                    this.myPost(function(response){
                        _this.find_client_list = response
                    },{acc_no:_this.find_a_client.client_id},this.url('/find/client'));
                }
            }
        }
    }
